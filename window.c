...
void add_packet_to_buffer(tcp_packet *pkt)
{
    ...
    // Create a copy of the packet for the buffer
    tcp_packet *pkt_copy = malloc(TCP_HDR_SIZE + pkt->hdr.data_size);
    if (pkt_copy == NULL) {
        printf("Warning: Failed to allocate memory for packet copy\n");
        return;
    }
    memcpy(pkt_copy, pkt, TCP_HDR_SIZE + pkt->hdr.data_size);
    
    // Store the packet copy pointer in the empty slot
    buffer[empty_slot] = pkt_copy;
    ...
    if (buffer[global_window->smallest_seqno_idx] == NULL || 
        pkt_copy->hdr.seqno < buffer[global_window->smallest_seqno_idx]->hdr.seqno) {
        ...
        if (buffer[i] != NULL && buffer[i]->hdr.seqno == seqno) {
            ...
            if (i == global_window->smallest_seqno_idx) {
                ...
                if (found_new_smallest) {
                    ...
                    // No packets left, reset smallest_seqno_idx
                    global_window->smallest_seqno_idx = 0;
                }
            }
